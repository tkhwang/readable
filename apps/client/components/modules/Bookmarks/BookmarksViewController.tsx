...
export const BookmarksViewController: ViewController<BookmarksViewModel> = React.memo(({ viewModel }) => {
  ...
  // <BookmarkCard title="My Bookmarks" bookmarks={myBookmarks} /> *
  ...
  return (
    <>
      ...
      <p className="text-2xl">Anonymous Bookmarks</p>
      {anonymousBookmarks?.length > 0 &&
        anonymousBookmarks.map(bookmark => {
          return (
            <a href={bookmark.url}>
              <div className="p-10">
                <div className="max-w-sm rounded overflow-hidden shadow-lg">
